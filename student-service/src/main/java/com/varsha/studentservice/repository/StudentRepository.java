...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    List<Student> findByStatusNotAndCounselorUsername(String status, String counselorUsername);
    List<Student> findByCounselorUsernameAndClassNameAndBranchAndSection(String counselorUsername, String className, String branch, String section);
    ...
    @Query("SELECT s FROM Student s WHERE s.status <> :status " +
           "AND (:counselorUsername IS NULL OR s.counselorUsername = :counselorUsername) " +
           "AND (:search IS NULL OR :search = '' OR LOWER(s.rollNumber) LIKE LOWER(CONCAT('%', :search, '%')) " +
           "OR LOWER(s.firstName) LIKE LOWER(CONCAT('%', :search, '%')) " +
           "OR LOWER(s.lastName) LIKE LOWER(CONCAT('%', :search, '%')) " +
           "OR LOWER(s.className) LIKE LOWER(CONCAT('%', :search, '%')) " +
           "OR LOWER(s.branch) LIKE LOWER(CONCAT('%', :search, '%')) " +
           "OR LOWER(s.section) LIKE LOWER(CONCAT('%', :search, '%')))")
    Page<Student> searchStudentsWithCounselor(
            @Param("status") String status,
            @Param("counselorUsername") String counselorUsername,
            @Param("search") String search,
            Pageable pageable
    );
}
